...
description: All React hooks from @tango/instrument-sdk/react.
---

All hooks are imported from `@tango/instrument-sdk/react`.
...
Import from `@tango/instrument-ui/react`:

```tsx
import { useInstrumentUIStyles } from "@tango/instrument-ui/react";
```
