...
**Enables**: Subscription to `stage.added`, `stage.removed`, and `stage.selected` events

React to stage changes in real time. The `stage.selected` event fires when a stage's metadata is loaded or refreshed, providing branch name, HEAD SHA, and change counts. Note: you can always subscribe to these events without this permission, but the host may throttle or filter them.
