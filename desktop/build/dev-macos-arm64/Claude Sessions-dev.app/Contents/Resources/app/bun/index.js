...
class SessionManager {
  ...
  isAppSpawned(sessionId) {
    return this.#sessions.has(sessionId);
  }
  async#writeToProc(proc, data) {
...
function parseEntry(entry) {
  ...
  if (role === "assistant" && entry.message?.content && Array.isArray(entry.message.content)) {
    const toolUse = entry.message.content.find((b) => b.type === "tool_use");
    if (toolUse) {
...
async function getSnapshotDiffFromBaseline(cwd, baseline) {
...
async function captureWorkspaceSnapshot(cwd) {
  const files = new Map;
  await walkDir(cwd, cwd, files);
  return files;
}
async function walkDir(root, dir, files) {
  let entries;
  try {
...
function makeAddedFile(path, content) {
...
function makeDeletedFile(path, content) {
  const lines = content.split(`
`);
  const diffLines = lines.map((line, i) => ({
    type: "delete",
...
function makeModifiedFile(path, oldContent, newContent) {
...
function buildHunk(_oldLines, _newLines, allEdits, hunkEdits, context) {
...
function myersDiff(oldLines, newLines) {
  ...
  if (n + m > 1e4) {
    return [
      ...oldLines.map((_, i2) => ({ type: "delete", oldIdx: i2, newIdx: -1 })),
      ...newLines.map((_, i2) => ({ type: "insert", oldIdx: -1, newIdx: i2 }))
    ];
  }
...
async function listSessionsForWorkspace(cwd) {
  ...
  const jsonlFiles = entries.filter((f) => f.endsWith(".jsonl") && !f.includes(".") ? false : f.endsWith(".jsonl"));
  const sessions = [];
...
async function parseTranscriptMeta(sessionId, filePath) {
  ...
  async load() {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        this.#workspaces = data.filter((w) => typeof w === "string");
      }
    } catch {
      this.#workspaces = [];
    }
...
// src/bun/approval-server.ts
class ApprovalServer {
  #pending = new Map;
  #onRequest = null;
  #server = null;
  #managedSessions = new Set;
  #autoAllowSessions = new Set;
  onApprovalRequest(cb) {
    this.#onRequest = cb;
    return this;
  }
  registerSession(sessionId, fullAccess = false) {
  ...
  resolveSessionId(tempId, realId) {
    ...
    if (wasManaged)
      this.#managedSessions.delete(tempId);
    if (wasAutoAllow)
      this.#autoAllowSessions.delete(tempId);
    if (wasManaged)
      this.#managedSessions.add(realId);
    if (wasAutoAllow)
      this.#autoAllowSessions.add(realId);
  }
      ...
      fetch: async (req) => {
        ...
        if (req.method === "POST" && url.pathname === "/api/tool-approve") {
          try {
            ...
            console.log(`
` + "\u2500".repeat(60));
            console.log(`[tool-hook] PreToolUse ${toolName ? `\u2192 ${toolName}` : ""} (session: ${String(sessionId).slice(0, 8)}\u2026)`);
            console.log(`[tool-hook-meta] toolUseId=${toolUseId || "\u2014"} managed=${this.#managedSessions.has(sessionId)} autoAllow=${this.#autoAllowSessions.has(sessionId)}`);
            console.log("[tool-hook-payload]");
            console.log(safeStringify2(body));
            if (!toolUseId) {
            ...
            return new Promise((resolveResponse) => {
              const timeout = setTimeout(() => {
                ...
                resolveResponse(new Response(JSON.stringify({ allow: true }), {
                  headers: { "Content-Type": "application/json" }
                }));
              }, 120000);
  ...
  respond(toolUseId, allow) {
    const pending = this.#pending.get(toolUseId);
    if (!pending)
      return;
    pending.resolve(new Response(JSON.stringify({ allow }), {
      headers: { "Content-Type": "application/json" }
    }));
    this.#pending.delete(toolUseId);
  }
  stop() {
    for (const [id, pending] of this.#pending) {
      pending.resolve(new Response(JSON.stringify({ allow: true }), {
        headers: { "Content-Type": "application/json" }
      }));
    }
    this.#pending.clear();
...
async function addHookToSettings(hookPath) {
  let settings = {};
  try {
    const content = await readFile5(CLAUDE_SETTINGS_PATH, "utf-8");
  ...
  const alreadyInstalled = hookGroups.some((group) => group?.hooks?.some((h) => h.command?.includes(HOOK_SCRIPT_NAME)));
  if (alreadyInstalled) {
...
class SessionNamesStore {
  ...
  async load() {
    try {
      const content = await readFile6(STORE_PATH, "utf-8");
...
var watcher = new WatcherClient;
var sessions = new SessionManager;
var workspaces = new WorkspaceStore;
var approvals = new ApprovalServer;
var sessionNames = new SessionNamesStore;
var latestSnapshot = null;
var mainRPC = null;
var sessionCwds = new Map;
async function ensureServer() {
  ...
  const serverPath = process.env.CLAUDE_WATCHER_SERVER ?? `${process.env.HOME}/Desktop/claude-watcher/src/server.js`;
  try {
  ...
  handlers: {
    requests: {
      ...
      getTranscript: async ({ sessionId, transcriptPath }) => {
        const path = transcriptPath ?? latestSnapshot?.tasks.find((t) => t.sessionId === sessionId)?.transcriptPath;
        if (!path)
          return [];
...
async function handleSessionEvent(sessionId, event) {
...
exports_ApplicationMenu.setApplicationMenu([
  {
    submenu: [
      { label: "About Claude Sessions", role: "about" },
      { type: "separator" },
